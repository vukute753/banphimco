    ...
    <form enctype="multipart/form-data" method="post">
        ...
        <div class="form_group">
            ...
            <select name="loai">
                <?php
                // $dsloai = layLoai($conn);
                $sqlLoai = "SELECT l.maloai, l.tenloai FROM loai l GROUP BY l.maloai";
                $dsloai = mysqli_query($conn, $sqlLoai);

                while ($rowloai = mysqli_fetch_array($dsloai)) {
                ?>
                    <option value="<?php echo htmlspecialchars($rowloai['maloai']); ?>" <?php if ($row['loai'] == $rowloai['maloai']) echo 'selected'; ?>>
                        <?php echo htmlspecialchars($rowloai['tenloai']); ?>
                    </option>
                <?php
                }
                ?>
